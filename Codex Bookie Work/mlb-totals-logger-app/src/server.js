...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/report") {
      return json(res, 200, await report());
    }
    if (req.method === "GET" && url.pathname === "/api/export.csv") {
      const body = await csvLedgerText();
      res.writeHead(200, {
        "content-type": "text/csv; charset=utf-8",
        "content-disposition": "attachment; filename=\"mlb_2026_totals_3am_game_line_ledger.csv\"",
        "cache-control": "no-store"
      });
      return res.end(body);
    }
    if (req.method === "POST" && url.pathname === "/api/get-data") {
      return json(res, 200, { ok: true, update: await getData() });
    }
    if (req.method === "GET") {
...
server.listen(port, host, () => {
  console.log(`Codex Bookie MLB Totals Logger running at http://${host}:${port}`);
});
